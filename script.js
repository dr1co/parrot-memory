let cartasQtd = Number(
  prompt("Com quantas cartas deseja jogar? (coloque um número entre 4 e 14)")
);
let jogadas = 0;
let tempo = 0;
...
function iniciarJogo() {
  let cartas = [
    `<img class="frente" src="./media/bike-parrot.gif" />`,
    `<img class="frente" src="./media/bunny-parrot.gif" />`,
    `<img class="frente" src="./media/parrot-explosion.gif" />`,
    `<img class="frente" src="./media/parrot-party.gif" />`,
    `<img class="frente" src="./media/red-parrot.gif" />`,
    `<img class="frente" src="./media/rock-parrot.gif" />`,
    `<img class="frente" src="./media/thinking-parrot.gif" />`,
  ];
  cartas.sort(comparador);

  const versoCarta = `<img class="verso" src="./media/front.png" />`;
  ...
  for (let i = 0; i < cartasQtd / 2; i++) {
    dequeCartas.push(
      `<li onclick="virarCarta(this)">${cartas[i] + versoCarta}</li>`
    );
    dequeCartas.push(
      `<li onclick="virarCarta(this)">${cartas[i] + versoCarta}</li>`
    );
...
function desvirarCartasCorretas() {
  ...
  if (cartasQtd === 0) {
    ...
    if (resposta === "sim") {
      cartasQtd = Number(
        prompt(
          "Com quantas cartas deseja jogar? (coloque um número entre 4 e 14)"
        )
      );
      jogadas = 0;
      tempo = 0;
